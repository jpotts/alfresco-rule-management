...
(function()
{
   ...
   YAHOO.extend(Alfresco.RulesHeader, Alfresco.component.Base,
   {
      ...
      options:
      {
         ...
         /** Are rules linked from elsewhere?
          *
          *
          * @property isLinked
          * @type Boolean
      ...
      onReady: function RulesHeader_onReady()
      {
         ...
         if(!this.options.isLinked) {
            this.widgets.convertToSharedButton = Alfresco.util.createYUIButton(this, "convertToShared-button", this.onConvertToSharedClick);
         } else {
            Dom.addClass(this.id + "-convertToShared-button", "hidden");
         }
      ...
      onConvertToSharedClick: function RulesHeader_onConvertToSharedClick() {

        console.log(this);

        var rulesObject = this;

       Alfresco.util.PopupManager.getUserInput(
         {
            title: this.msg("message.convertShared-confirm"),
            value: this.folderDetails.fileName,
            input: "text",
            callback: {
                fn: function(folderName) {
                    ...
                    Alfresco.util.Ajax.jsonPost(
                     {
                        url: Alfresco.constants.PROXY_URI_RELATIVE + "metaversant/share-rule",
                        dataObj:
                        {
                           "sourceFolderName" : folderName,
                           "nodeRef": Alfresco.util.NodeRef(rulesObject.options.nodeRef).toString()
                        },
                        successCallback:
                        {
                        ...
                        failureCallback:
                        {
                           fn: function(response)
                           {
                              ...
                              var msg = rulesObject.msg("message.convertShared-failure");
                              ...
                              {
                                 title: rulesObject.msg("message.failure"),
                                 text: msg
                              });
                           },
                           scope: rulesObject
